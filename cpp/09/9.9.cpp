...
void DateAndTime::setDate(int m, int d, int y) {
    ...
    if (month <= 0 || month > 12) throw invalid_argument("invalid argument month");
    if (day <= 0 || day > max_day()) throw invalid_argument("invalid argument day");
}
...
void DateAndTime::printUniversal() const {
    print();
    cout << ' ' << setfill('0') << setw(2) << getHour() << ":" << setw(2) << getMinute() << ":" << setw(2)
         << getSecond();
}
...
void DateAndTime::printStandard() const {
    ...
    cout << ' ' << ((getHour() == 0 || getHour() == 12) ? 12 : getHour() % 12) << ":" << setfill('0') << setw(2)
         << getMinute() << ":" << setw(2) << getSecond() << (hour < 12 ? " AM" : " PM");
}
...
unsigned int DateAndTime::max_day() const {
    static const unsigned int days[13] = {0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
