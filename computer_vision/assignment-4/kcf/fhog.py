...
def func1(dx, dy, boundary_x, boundary_y, height, width, numChannels):
    ...
    for j in xrange(1, height - 1):
        for i in xrange(1, width - 1):
            ...
            # 对其余通道计算通道，如果在其余的通道的梯度幅值大于目前通道梯度
            # 则更新r[j,i]以及横向与竖向的梯度值
            ...
            for kk in xrange(0, NUM_SECTOR):
                # 横向与竖向的梯度值乘以各个角度的cos和sin,如果这个值最大，说明理这个方向最近
...
def getFeatureMaps(image, k, mapp):
    ...
    sizeX = width // k
    sizeY = height // k
    px = 3 * NUM_SECTOR  # 27 个人推测9个方向，三张图？
    p = px  # 27
    stringSize = sizeX * p

    mapp['sizeX'] = sizeX  # x方向有多少个cell
    mapp['sizeY'] = sizeY  # y方向有多少个cell
    mapp['numFeatures'] = p  # 每个cell有多少个feature
    ...
    # 将一个π九等分，获取等分的数值，[0. 0.34906587 0.69813174 1.0471976  1.3962635  1.7453294
    #                           2.09439522.443461 2.792527 3.1415927 ]
    arg_vector = np.arange(NUM_SECTOR + 1).astype(np.float32) * np.pi / NUM_SECTOR
    # cos化 [ 1. 0.9396926   0.76604444  0.49999997  0.1736481  -0.1736483, -0.50000006 -0.7660446  -0.9396927  -1.        ]
    boundary_x = np.cos(arg_vector)
    # sin化 [ 0.0000000e+00  3.4202015e-01  6.4278764e-01  8.6602545e-01,  9.8480779e-01  9.8480773e-01  8.6602539e-01  6.4278746e-01,  3.4202000e-01 -8.7422777e-08]
	...
	'''
    ### 200x speedup
    # alfa是在每个像素在哪个方向，第一个通道%了9，将大小固定在0到8之间，而第二个通道没有
